...
test("builds protocol-appropriate model list URLs", () => {
  ...
  assert.equal(buildModelsListUrl("https://api.example.com/custom/models/#private", "openai-responses").toString(), "https://api.example.com/custom/models");
});
...
test("distinguishes explicit challenges, HTML pages, and non-JSON responses", async () => {
  ...
  await expectDiscoveryError(fetchDiscoveryJson(
    new URL("https://api.example.com/v1/models"),
    discoveryOptions(async () => new Response("<html>login</html>", {
...
test("route returns only the sanitized models endpoint for discovery failures", async () => {
  ...
  try {
    const response = await discoverModels(new Request("http://localhost:30141/api/models-config/discover", {
      method: "POST",
      headers: { host: "localhost:30141", "content-type": "application/json" },
      body: JSON.stringify({
        providerName: "test-provider",
        provider: {
          baseUrl: "https://api.example.com/v1?api_key=super-secret-key#private",
...
test("maps authentication, rate-limit, and server failures without exposing bodies", async () => {
  for (const [status, code, contentType] of [
    [401, "UPSTREAM_AUTH_FAILED", "application/json"],
    [403, "UPSTREAM_AUTH_FAILED", "text/html"],
    [404, "UPSTREAM_HTTP_ERROR", "text/html"],
    [429, "UPSTREAM_RATE_LIMITED", "application/json"],
    [503, "UPSTREAM_UNAVAILABLE", "text/html"],
  ]) {
    await expectDiscoveryError(fetchDiscoveryJson(
      new URL("https://api.example.com/v1/models"),

...
test("permission modes gate impactful tools at the SDK execution boundary", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const controllerSource = source.slice(
    source.indexOf("class PermissionController"),
    source.indexOf("// Extensions require a complete Theme"),
  );
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  assert.match(controllerSource, /pi\.on\("tool_call"/);
  assert.match(controllerSource, /READ_ONLY_TOOL_NAMES\.has\(event\.toolName\)/);
  assert.match(controllerSource, /mode === "restricted"/);
  assert.match(controllerSource, /decision === "allow_session"/);
  assert.match(controllerSource, /TOOL_APPROVAL_TIMEOUT_MS/);
  assert.match(startupSource, /extensionFactories: \[createPermissionExtension\(permissionController\)\]/);
});

test("permission approval requests survive SSE reconnects and are denied on teardown", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const onEventSource = source.slice(source.indexOf("  onEvent(listener"), source.indexOf("  onDestroy"));
  const destroySource = source.slice(source.indexOf("  destroy(): void"), source.indexOf("  async shutdown"));

  assert.match(onEventSource, /permissionController\.getPendingEvents\(\)/);
  assert.match(destroySource, /permissionController\.cancelPending\("Session was closed\."\)/);
  assert.match(source, /case "tool_approval_response"/);
});

...
test("restores an unsaved workspace cwd when there is no persisted session", async () => {
  const { resolveInitialNavigation } = await loadSubject();
  assert.deepEqual(
    resolveInitialNavigation(
      new URLSearchParams(),
      { sessionId: null, cwd: "/tmp/no-project", fileTabs: [], activeFileTabId: null, rightPanelOpen: false },
    ),
    { requestedCwd: "/tmp/no-project", sessionId: null },
  );
});

...
test("the internal no-project workspace never loads project resources", async (t) => {
  const { agentDir } = await createProjectFixture(t);
  const cwd = join(agentDir, "workspace");
  await mkdir(join(cwd, ".agents", "skills", "project-only"), { recursive: true });
  await writeFile(
    join(cwd, ".agents", "skills", "project-only", "SKILL.md"),
